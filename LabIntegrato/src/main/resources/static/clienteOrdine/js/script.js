// Definizione globale dei prodotti
        ...
        async function fetchProdotti() {
            try {
                const token = localStorage.getItem('token'); // Sostituisci 'token' con la tua chiave effettiva
                if (!token) {
                    throw new Error('Token non trovato. Autenticazione richiesta.');
                }

                const response = await fetch('/cliente/prodotti', { // Sostituisci con il tuo endpoint reale
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json' // Opzionale, a seconda delle tue esigenze
                    }
                });
        ...
        async function fetchProdotti() {
            try {
                const token = localStorage.getItem('token'); // Sostituisci 'token' con la chiave effettiva se diversa
                if (!token) {
                    throw new Error('Token non trovato. Autenticazione richiesta.');
                }

                const response = await fetch('/cliente/prodotti', {
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
        ...
        function addToCart(productId) {
            ...
            if (quantityToAdd > 0) {
                ...
                if (existingItem) {
                    ...
                    existingItem.total = existingItem.quantity * existingItem.prezzo;
                    ...
                    cartItems.push({
                        ...
                        total: prodotto.prezzo * quantityToAdd,
                        toProduce: toProduce
                    });
                }
            ...
            cartItems.forEach(item => {
                ...
                itemElement.innerHTML = `
                    <div class="product-info">
                        ...
                        ${item.toProduce > 0 ? `<p style="color: #e74c3c; font-size: 12px;">${item.toProduce} pezzo/i da produrre</p>` : ''}
                    </div>
                    <div class="quantity">
        ...
        async function verifyToken() {
            try {
                ...
                // (Opzionale) Verifica il token con il server se necessario
                /*
                const response = await fetch('/cliente/verify-token', { 
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                ...
                console.log('Token verificato con successo:', data);

                // Controlla il ruolo dalla risposta del server
                ...
                */
                ...
                Swal.fire({
                    ...
                    window.location.href = '/public/login'; // Sostituisci con il percorso della tua pagina di login
                });
            }
        }
